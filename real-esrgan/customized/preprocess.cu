...
// preprocess (IYUV->RGB444, [0, 255]->[0, 1](Normalize))
__global__ void PreprocessKernel(
    const unsigned char* __restrict__ srcY,  // input Y plane
    const unsigned char* __restrict__ srcUV, // input UV interleaved plane
    float* __restrict__ dstBuffer,           // TRT input buffer (NCHW, RGB, [0, 1])
    int srcWidth, int srcHeight,

    float* output, uint8_t* input,
