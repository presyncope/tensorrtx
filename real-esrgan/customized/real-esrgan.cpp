...
ICudaEngine* build_engine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, std::string& wts_name) {
    NetworkDefinitionCreationFlags flags = 0u;
    flags |= (uint32_t)NetworkDefinitionCreationFlag::kSTRONGLY_TYPED;
    ...
    // Custom preprocess (IYUV->RGB444, [0, 255]->[0, 1](Normalize))
...
int main(int argc, char** argv) {
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(buffers[inputIndex], input.data(),
                                   BATCH_SIZE * INPUT_C * INPUT_H * INPUT_W * sizeof(uint8_t), cudaMemcpyHostToDevice,
                                   stream));
